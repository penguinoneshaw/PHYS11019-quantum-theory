...
A forced \emph{an}harmonic oscillator has an action \(S = S_0[x]_J\footnote{I'm not certain about this notation, but I know for sure and certain I don't like the one in the notes. It's alsoc significantly different to the one I used earlier.} + S_1[x]\) with Lagrangian \[L = L_{\text{Driven SHO}} - \frac{\lambda}{4}x^4 \] and
...
With this in mind, we build a Green's function \(\Delta(t, t^\prime)\) such that \[(\pdv[2]{t} + \omega^2) \Delta(t, t^\prime) = -\delta(t - t^\prime) \] with boundary conditions \( \Delta(t_a, t^\prime) = \Delta(t_b, t^\prime) = 0 \).
